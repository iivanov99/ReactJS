...
module.exports = {
  get: {
    all: async (req, res) => {
      ...
      let accessories = limit
        ? await models.Accessory.find({}).sort({ ordersCount: -1 }).limit(parseInt(limit))
    ...
    one: async (req, res, next) => {
      try {
        const { id } = req.params;
        const accessory = await models.Accessory.find({ _id: id });
        res.json(accessory);
      } catch (err) {
        next(err);
      }
  ...
  post: async (req, res, next) => {
    try {
      ...
      const createdAccessory = await models.Accessory
  ...
  put: async (req, res, next) => {
    try {
      const { id } = req.params;
      const { name, category, price, description, size, imageUrl } = req.body;
      const updatedAccessory = await models.Accessory
